...
export default function Login(){
  ...
  const submit=async e=>{e.preventDefault();setState('sending');try{const r=await fetch('/api/auth/magic-link',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})});const d=await r.json();if(!r.ok)throw Error(d.message||d.error);setState('sent')}catch(e){setState(e.message||'error')}};
