...
return <main className="page finalPage unlimitedFinal">{termSheetEl}<Link to="/ujian" className="back">← Ujian Akhir</Link><div className="unlimitedHero"><p className="eyebrow">UNLIMITED PRACTICE</p><h1>Latihan tanpa batas</h1><p className="muted">Soal terus berputar. Tidak memengaruhi skor resmi, XP, atau progress ujian.</p><span className="quizModeBadge">{count>=100?'Maraton 100 selesai ➤✦':`${count}/100 soal — maraton`}</span></div><LangPicker lang={lang} setLang={setLang}/><section className="finalQuestion"><small>Soal latihan #{q} · {item.exam} 問題{item.no} · {item.subject}</small><Furigana field={item.prompt} mode={lang} as="h2" glossary={gloss} onTerm={openTerm}/>{item.image&&<img className="finalQFigure" src={`/assets/final/${item.image}`} alt="図"/>}<div className="finalOptions">{item.options.map(o=><button className={selected===o.key?'selected':''} key={o.key} onClick={()=>choose(o.key)}><b>{o.key}</b><Furigana field={o.text} mode={lang} variant="opt" glossary={gloss} onTerm={openTerm}/></button>)}</div>{selected&&<div className="finalFeedback">{isCorrectAnswer(item,selected)?'Benar ✦':'Coba lagi ya'}<br/>{item.explanation.id}</div>}</section><button className="primary big" onClick={()=>{setQ(q+1);setSelected(null)}}>Soal berikutnya <ChevronRight/></button></main>}
...
return <main className="page finalQuiz">{termSheetEl}<div className="finalQuizTop"><Link to={`/ujian/${year}`}>× Tutup</Link><span>問題 {q.no} · {qi+1}/25</span><span className={mode==='practice'?'quizModeTag quizModeTagPractice':'quizModeTag'}>{mode==='practice'?'Latihan':'Ujian'}</span></div><LangPicker lang={lang} setLang={setLang}/><div className="finalQuestion"><Furigana field={q.prompt} mode={lang} as="h2" glossary={gloss} onTerm={openTerm}/>{q.image&&<img className="finalQFigure" src={`/assets/final/${q.image}`} alt="図"/>}<div className="finalOptions">{q.options.map(o=><button className={answers[q.no]===o.key?'selected':''} key={o.key} onClick={()=>choose(o.key)}><b>{o.key}</b><Furigana field={o.text} mode={lang} variant="opt" glossary={gloss} onTerm={openTerm}/></button>)}</div>{mode==='practice'&&answers[q.no]&&<div className="finalFeedback">{isCorrectAnswer(q,answers[q.no])?'Benar':'Cek lagi'} — {q.explanation.id}</div>}</div>{submitErr&&<div className="submitError">{submitErr}</div>}<div className="finalQuizNav"><button disabled={qi===0} onClick={()=>setQi(qi-1)}>Sebelumnya</button>{qi<24?<button onClick={()=>setQi(qi+1)}>Berikutnya</button>:<button onClick={submit}>Kirim bagian</button>}</div></main>}
